...
- Docs: <http://localhost:8033/docs>
- Admin: <http://localhost:8033/admin> — `admin@example.com` / `admin123` after `make seed`
- Frontend: <http://localhost:3033> — start with `make dev-frontend` (Docker) or `cd frontend && bun install && bun dev` (local)
...
└── commands/             # Click CLI commands (auto-discovered by `ai_agent cmd …`)
...
The generated project ships a Click CLI exposed as `ai_agent` (after `make install`):

```bash
ai_agent server run --reload          # dev server
ai_agent db upgrade                   # apply migrations
ai_agent db migrate -m "message"      # create new migration
ai_agent user create-admin            # interactive admin creation
ai_agent rag-ingest <path> -c docs    # ingest local files
ai_agent rag-search "query" -c docs   # semantic search
ai_agent rag-collections              # list collections
```

Run `make help` for a categorized list, or `ai_agent --help` for full CLI docs.
...
ai_agent rag-ingest /path/to/docs/ --collection documents --recursive

# Semantic search
ai_agent rag-search "your query" --collection documents
```
...
The frontend talks to the backend through Next.js API route handlers in `src/app/api/*` (server-side proxy that forwards auth cookies to the FastAPI backend). Direct calls to `localhost:8033` from the browser are deliberately avoided.
