...
```bash
uv run ai_agent taskiq worker                # Start worker
uv run ai_agent taskiq worker --workers 4    # 4 worker processes
uv run ai_agent taskiq worker --reload       # With auto-reload (dev)
uv run ai_agent taskiq scheduler             # Start periodic scheduler
```
